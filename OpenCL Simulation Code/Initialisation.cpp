...
void SimulationInitialisation::InitialiseSimulation(MicroscopeParameters* params, int res, MultisliceStructure* Structure, float startx, float starty, float endx, float endy, bool Full3D, bool FD, float dz, int full3dints, int waves = 1)
{
	//set some variables we need to keep
	mParams.reset(params);
	isFD = FD;
	resolution = res;
	AtomicStructure.reset(Structure);
	...
	float wavelength = mParams->Wavelength(); // Electron wavelength (Angstrom)
