__kernel void clBinnedAtomicPotentialOpt2(__global float2* Potential, 
										  ...
										  __global const int* restrict clBlockStartPositions, int width, int height, int slice, int slices, float z, float dz, float pixelscale, int xBlocks, int yBlocks, float MaxX, float MinX, float MaxY, float MinY, int loadBlocksX, int loadBlocksY, int loadSlicesZ, float sigma)
{
	...
	__local int atZ[256];
	__local int starts[1024];
	__local int ends[1024];
	...
	int numberofz = (bottomz-topz + 1);
	int numberofy = (endj-startj+1);

	for(int a = lid; a < numberofz*numberofy; a+=256)
	{
		int j = a%numberofy;
		int k = (a-j)/numberofy;	
		starts[a] = clBlockStartPositions[(k+topz)*xBlocks*yBlocks + xBlocks*(j+startj) + starti];
		ends[a] = clBlockStartPositions[(k+topz)*xBlocks*yBlocks + xBlocks*(j+startj) + endi + 1];
	}

	barrier(CLK_LOCAL_MEM_FENCE);
	
	for(int m = 0; m < numberofz*numberofy; m++)
	{			
		int gid = starts[m] + lid;

		if(lid < ends[m]-starts[m])
		{
		...
		float p2=0;
		for (int l = 0; l < ends[m]-starts[m]; l++) 
		{
			int ZNum = atZ[l];
			for (int h = 0; h <= 10; h++)
			{
