...
void CTEMSimulation::initialiseCTEMSimulation(MicroscopeParameters* params, MultisliceStructure* Structure, int res, float startx, float starty, float endx, float endy, bool Full3D, bool FD, float dz, int full3dints)
{
	...
	InitPlaneWavefunction = clKernel(UnmanagedOpenCL::ctx, InitialiseWavefunctionSource.c_str(), 4, "clInitialiseWavefunction");
	...
	initialisePlaneWavefunction();
}

void CTEMSimulation::initialisePlaneWavefunction()
{
...
void CTEMSimulation::simulateCTEM(int detector, int binning, float doseperpix, float conversionfactor)
{
	...
	auto dqentfbuffer = UnmanagedOpenCL::ctx.CreateBuffer<cl_float, Manual>(725);
	...
	// write DQE to opencl
	clEnqueueWriteBuffer(UnmanagedOpenCL::ctx.GetIOQueue(), dqentfbuffer->GetBuffer(), CL_TRUE, 0, 725 * sizeof(float), dqes[detector], 0, NULL, NULL);
	// apply DQE
...
void CTEMSimulation::getEWImage(float* data, int resolution, int wave)
{
	// Original data is complex so copy complex version down first
...
void CTEMSimulation::getEWImage2(float* data, int resolution, int wave)
{
	// Original data is complex so copy complex version down first
	std::vector<cl_float2> compdata = clWaveFunction1[wave - 1]->CreateLocalCopy();
