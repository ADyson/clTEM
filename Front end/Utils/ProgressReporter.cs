...
namespace SimulationGUI
{
    class ProgressReporter
    {
        ...
        public Task ReportProgressAsync(Action<object> action, float val)
        {
            return Task.Factory.StartNew(action, val, CancellationToken.None, TaskCreationOptions.None, this.scheduler);
        }
        ...
        public void ReportProgress(Action<object> action, float val)
        {
            this.ReportProgressAsync(action, val).Wait();
        }
        ...
        public Task RegisterSucceededHandler(Task task, Action action)
        {
            return task.ContinueWith(_ => action(), CancellationToken.None,
                TaskContinuationOptions.OnlyOnRanToCompletion, this.scheduler);
        }
        ...
        public Task RegisterFaultedHandler(Task task, Action<Exception> action)
        {
            return task.ContinueWith(t => action(t.Exception), CancellationToken.None,
                TaskContinuationOptions.OnlyOnFaulted, this.Scheduler);
        }
